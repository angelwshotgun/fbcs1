...
def api_create_teams():
    """Thuật toán chia 10 người chơi thành 2 đội cân bằng nhất."""
    try:
        data = request.json or {}
        ...
        result = matchmaking_service.create_balanced_teams(players)
...
def api_create_teams_with_captains():
    """Thuật toán chia đội với 2 Đội trưởng và 8 tuyển thủ."""
    try:
        data = request.json or {}
...
def api_update_match_result():
    """Ghi nhận kết quả trận đấu, tự động cập nhật Elo ẩn và tính lại Phong độ."""
    try:
        data = request.json or {}
        ...
        return jsonify({
            'success': True,
            'message': 'Đã lưu kết quả trận đấu. Elo ẩn và Phong độ đã được cập nhật tự động!'
        }), 200
    except Exception as e:
...
def api_get_synergies():
    """Lấy danh sách các cặp bài trùng có tỷ lệ thắng cao nhất."""
    try:
        ...
        return jsonify({'success': True, 'synergies': pairs_list}), 200
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
